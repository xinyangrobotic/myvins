...
//                        X_p += vertex.second->GetX_p();
//                    }
//                    cout << "X_p: \n" << X_p << endl;
//                    if (delta_gn_a_x_.norm() <= epsilon2_ * X_p.norm()) {
//                        stop = true;
//                        break;
//                    } else {
                        UpdateStates();
...
//        if (sqrt(currentChi_) < 1e-15)
                if (last_chi_ - currentChi_ < 1e-5) {
                    std::cout << "sqrt(currentChi_) <= stopThresholdDL_" << std::endl;
                    stop = true;
                }
...
/// DL
        void Problem::ComputeLambdaInitDL() {
            ...
            stopThresholdDL_ = 1e-10 * currentChi_;          // 迭代条件为 误差下降 1e-6 倍
...
//    scale += 1e-3;    // make sure it's non-zero :)
        ...
        bool Problem::IsGoodStepInDL() {
            ...
            double rho = (currentChi_ - tempChi) / (2 * currentChi_ - (2 * (currentChi_ - b_.transpose() * delta_x_ +
                                                                            0.5 * delta_x_.transpose() * Hessian_ *
                                                                            delta_x_)));
            if (rho > 0 && isfinite(tempChi)) {   // last step was good, 误差在下降
                UpdateRadius(rho);
                return true;
